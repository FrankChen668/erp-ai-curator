...
8. **准备返回 0 推荐时，至少换一种来源类型或搜索表达做一次反证式搜索，确认不是因为第一轮搜索路径过窄。** 不设固定查询次数，也不为了避免空缺无限搜索。

第 8 条来自真实稀缺主题的纠偏：第一次搜索 Oracle 顾问 AI 方法时过早判断为空，换到课程 / practitioner 来源后发现了高任务匹配候选。它是 abstain 前的反证习惯，不是新的 Gate。
