...
- [Controlled User Trial Guide](USER_TRIAL_GUIDE_V1.md) — **真实试用用户/管理员入口**；不要求理解项目历史。
- [Release Readiness — Adversarial](validation/RELEASE_READINESS_ADVERSARIAL_20260830.md) — `CONTROLLED USER TRIAL GO / BROAD RELEASE NO` 的发布裁决。
- [Curation Pack 01 Adversarial Review](validation/CURATION_PACK_01_ADVERSARIAL_REVIEW.md) — 四个高区分度真实来源 Case 的收口复审。
...
- Skill 0.7.0 runtime/Harness 已校准；
- Curation Pack 01 已覆盖 A、B、system-access boundary；
- 用户试用入口已具备；
- 可以停止内部预发布扩卡/润色，进入真实用户使用。
...
- [Case 001 — ERP 操作手册](curation-cases/CASE_001_ERP_OPERATING_MANUAL.md) — B；
- [Case 002 — Oracle EBS AI 开发](curation-cases/CASE_002_ORACLE_EBS_DEVELOPMENT.md) — B；
- [Case 003 — 多顾问周报汇总](curation-cases/CASE_003_WEEKLY_REPORT_CONSOLIDATION.md) — A；
- [Case 004 — SAP Bug 诊断/系统访问](curation-cases/CASE_004_SAP_BUG_DIAGNOSIS_SYSTEM_ACCESS.md) — A → conditional B。

它们可以证明：

- 产品确实从真实来源问题开始；
- Curator 能形成可审查的采用建议；
- A/B/system-access 边界在 0.7.0 下具有基本区分度。
...
可以把 0.7.0 给少量真实 ERP/企业信息化用户，在已知/批准的 Agent Skills 宿主里自然使用。
