...
- [0.8.1 Practitioner Execution Patch](validation/CURATOR_081_PRACTITIONER_EXECUTION_PATCH.md) — 当前最新 runtime 窄修正；依据 Codex Desktop 实际执行日志。
...
- [AI Leverage Model](AI_LEVERAGE_MODEL_V3.md) — 历史/分析层的详细判断模型；不作为 0.8.1 runtime contract 或每次运行必读材料。
...
- Skill **0.8.1** 保持 0.8.0 的 simplified runtime，并加入两条日志驱动的执行修正；
...
这些标签保留为当时分析记录，不再要求 0.8.1 runtime 先产生 A/B/C。
...
可以把 0.8.1 给少量真实 ERP/ToB/企业信息化用户，在已知/批准的 Agent Skills 宿主里自然使用。
